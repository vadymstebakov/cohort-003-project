...
import { getTeamAdminUserIds } from "./teamService";
...
function notifyTeamAdminsOfRedemption(opts: {
  ...
  const { teamId, courseId, redeemingUserId } = opts;
  ...
  const adminUserIds = getTeamAdminUserIds(teamId);
